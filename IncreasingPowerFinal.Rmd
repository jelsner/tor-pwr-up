...
c1 <- ggplot(df, aes(Hour, nT)) +
  ...
  theme_minimal()
<<<<<<< HEAD
c2 <- ggplot(df, aes(Hour, q50ED/10^9)) +
=======
c2 <- ggplot(df, aes(Hour, geometricMean/10^9)) +
>>>>>>> 94983b79f5a48a6ec8d91941b2f6313660339d55
  geom_bar(stat = "identity") +
  scale_x_continuous(breaks = seq(0, 23, 2), 
                     labels = labsHours) +
  coord_polar(start = -0.12) +
...
Putting plots together.  Thanks to https://github.com/thomasp85/patchwork/ If `library(patchwork)` from Thomas Pender returns an error. Install `ggplot2` from GitHub (Hadley Wickham). Restart your session.
```{r}

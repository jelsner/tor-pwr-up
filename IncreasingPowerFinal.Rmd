...
Put multiple plots on the same page using **patchwork**.  Thanks to https://github.com/thomasp85/patchwork/ If `library(patchwork)` from Thomas Lin Pedersen returns an error. Install `ggplot2` from GitHub (Hadley Wickham). Restart your session.
```{r}
...
ggplot(df, aes(x = Hour, y = perTorCas)) +
  ...
  labs(x = "Month", y = "Casualty Rate Per\n Casualty-Producing Tornado") +
  theme_minimal()
```
...
```{r}
yrep <- posterior_predict(model, nsamples = 5000)

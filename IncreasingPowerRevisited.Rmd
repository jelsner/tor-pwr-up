...
```{r}
Torn.sf <- Torn.sf %>%
  filter(yr >= 1995 & yr <= 2019) %>%
  filter(!st %in% c("AK", "HI", "PR")) %>%
  filter(wid != 0 | len != 0) %>%
  filter(mag >= 0) %>%
  mutate(Damage = factor(case_when(mag <= 1 ~ "Minor",
                                   mag >= 2 ~ "Significant"))) %>%
  ...
  mutate(Location = factor(case_when(Plains ~ "Plains",
                               Southeast ~ "Southeast",
                               !Plains & !Southeast ~ "Outside Plains & Southeast"),
                            levels = c("Plains", "Southeast", "Outside Plains & Southeast"))) %>%
...
In the period 1995-2006 the Southeast accounted for 25.7% of all tornadoes and the Plains 30.8% of all tornadoes. In the more recent period 2007-2018 the Southeast accounted for 31.8% of all tornadoes and the Plains 29.1%. Tornadoes in the Southeast tend to have longer tracks, with an average track length of 11.8 km over the period 2007-2018 compared with tornadoes in the Plains, with an average track length of 10.0 km over the same period. Thus a changes in path characteristics described in this paper are confounded by the well-known geographic shift in tornado activity.

557/2650 = 21% more tornadoes in the Southeast 2007-2018 compared with 1995-2006
723/6267 = 12% fewer tornadoes in the Plains 1995-2006 vs 2007-2018

635/1367 = 46% more EF1+ tornadoes in the Southeast 2007-2018 compared with 1995-2006
194/1636 = 12% more EF1+ tornadoes in the Plains 2007-2018 compared with 1995-2006
...
X <- Torn.sf %>% 
  ...
  summarize(avgLength = mean(Length),
            avgWidth = mean(Width)) %>%
  group_by(st) %>%
  mutate(GT = avgWidth[Epoch ==  "1995-2006"] > avgWidth[Epoch ==  "2007-2018"])
```

States where the average path length for EF1+ tornadoes was longer before the implementation of the EF rating scale in 2007: AR, DE, IA, IL, IN, KY, LA, MD, MO, MS, NY, OH, PA, SD, UT, VT, WV

States where the average path width for EF1+ tornadoes was wider before the implementation of the EF rating scale in 2007: DE, ID, IL, IN, KY, MI, PA, SD, TN, UT
...
wilcox.test(PerYear50 ~ Epoch, data = df2)
...
df3 <- Torn.sf %>%
  ...
  summarize(ratio21 = sum(mag >= 2)/sum(mag >= 1),
   ...
   mutate(Epoch = factor(yr >= 2007, labels = c("1995-2006", "2007-2018")),
          Location = factor(case_when(Plains ~ "Plains",
                               Southeast ~ "Southeast",
                               !Plains & !Southeast ~ "Outside Plains & Southeast"),
                            levels = c("Plains", "Southeast", "Outside Plains & Southeast")))
...
ggplot(data = df3,
  ...
  geom_point(data = df3[c(25, 50, 75),], aes(x = yr, y = Avg), color = "black", size = 2) +
  facet_wrap(~ Location, nrow = 3) +
  ggtitle(label = "No significant difference in the average count ratios before and after 2007",
            subtitle = "Ratio of tornado counts rated (E)Fn or worse to counts rated (E)Fn+1 or worse averaged for n = 1, 2, & 3.\nBefore (blue) and after the EF rating change in 2007") +
  theme_minimal()
...
p <- ggplot(data = df3,
  ...
  geom_point(size = 2, show.legend = FALSE) +
#  geom_smooth(show.legend = FALSE) +
  geom_segment(mapping = aes(x = 1995, xend = 2006, y = .243, yend = .243), color = "blue") +
  geom_segment(mapping = aes(x = 2007, xend = 2019, y = .219, yend = .219), color = "red") +
  geom_point(data = df3[25,], aes(x = yr, y = Avg), color = "black", size = 2) +
  ggtitle(label = "No significant difference in the average count ratios before and after 2007",
            subtitle = "Ratio of tornado counts rated (E)Fn or worse to counts rated (E)Fn+1 or worse averaged for n = 1, 2, & 3.\nBefore (blue) and after the EF rating change in 2007. The value for 2019 (black) is not included in Edwards et al.") +
  theme_bw()

p + geom_ysidedensity(aes(x = stat(density)), show.legend = FALSE)
```
...
Torn.sf %>%
  st_drop_geometry() %>%
  group_by(ThreeYearBins, Location, Damage) %>%
  summarize(AvgLength = mean(Length) / 1000,
            AvgWidth = mean(Width)) %>%
ggplot(aes(x = ThreeYearBins, y = AvgWidth)) +
  geom_point() +
  scale_y_continuous(limits = c(0, NA)) +
#  geom_smooth(method = lm) +
  facet_wrap(~ Location + Damage, ncol = 2) +
  theme_minimal()
```

Figure 7: Average path width by F & EF
```{r}
Torn.sf %>%
  st_drop_geometry() %>%
  group_by(mag, Epoch) %>%
  summarize(AvgWidth = mean(Width),
            AvgLength = mean(Length) / 1000) %>%
ggplot(aes(x = factor(mag), y = AvgLength)) +
  geom_col(aes(fill = Epoch), position = "dodge") +
  scale_fill_manual(values = c("blue", "red")) +
  scale_y_continuous(limits = c(0, 100)) +
  theme_minimal() +
  xlab("(E)F Rating") + ylab("Average Path length (km)") +
  ggtitle(label = "Damage paths are longer for tornadoes rated since the EF scale was implemented")

Torn.sf %>%
  st_drop_geometry() %>%
  filter(!Location == "Outside Plains & Southeast") %>%
  group_by(mag, Location) %>%
  summarize(AvgWidth = mean(Width),
            AvgLength = mean(Length) / 1000) %>%
ggplot(aes(x = factor(mag), y = AvgLength)) +
  ...
  theme_minimal() +
  xlab("(E)F Rating") + ylab("Average Path length (km)") +
  ggtitle(label = "Damage paths are longer for tornadoes occurring in the Southeast")
```
